...
const RadioOption = ({
    ...
    <label
        htmlFor={id}
        className={`flex cursor-pointer items-start gap-3 rounded-2xl border p-4 transition-all ${checked
                        ...
                        <section className="rounded-[32px] border border-white/60 bg-white/70 p-6 shadow-[0_20px_60px_rgba(15,23,42,0.12)] backdrop-blur-xl sm:p-8">
                            <div className="mb-8">
                                ...
                                <p className="mt-2 text-sm leading-6 text-slate-500">
                                    Enter your email below and we’ll send a one-time
                                    ...
                                    <div className="space-y-3">
                                        {otpOptions.map((option) => (
                                            <RadioOption
                                                key={option.id}
                                                id={option.id}
                                                checked={otpMethod === option.id}
                                                onChange={() => setOtpMethod(option.id)}
                                                title={option.title}
